...
app.post("/api/bikes", function(req, res) {
  ...
  mongoDB
    .collection(mongoDBCollection)
    .insertOne(newBike, function(err, result) {
      if (err) {
        dbError(res, err, requestID);
        return;
      }
    ...
    .updateOne(
      ...
      function(err, result) {
        ...
        if (result.matchedCount !== 1 && result.modifiedCount !== 1) {
          var msg =
            'Unexpected number of bikes modified! Matched: "' +
            result.matchedCount +
            '" Modified: "' +
...
app.get("/api/bikes/:bikeId", function(req, res) {
  ...
  mongoDB
    .collection(mongoDBCollection)
    .findOne({ _id: new ObjectId(req.params.bikeId) }, function(err, result) {
      if (err) {
        dbError(res, err, requestID);
        return;
      }
...
app.delete("/api/bikes/:bikeId", function(req, res) {
  ...
  mongoDB
    .collection(mongoDBCollection)
    .deleteOne({ _id: new ObjectId(req.params.bikeId) }, function(err, result) {
      if (err) {
        dbError(res, err, requestID);
        return;
      }
      ...
      if (result.deletedCount !== 1) {
        var msg =
          'Unexpected number of bikes deleted! Deleted: "' +
          result.deletedCount +
          '"';
        console.log(requestID + " - " + msg);
        res.status(500).send(msg);
...
function processReservation(res, bikeId, changeTo, requestID) {
  ...
  mongoDB
    .collection(mongoDBCollection)
    .updateOne(
      { _id: new ObjectId(bikeId), available: !changeTo },
      { $set: { available: changeTo } },
      function(err, result) {
        if (err) {
          dbError(res, err, requestID);
          return;
        ...
        if (result.matchedCount === 0) {
          ...
          mongoDB
            .collection(mongoDBCollection)
            .findOne({ _id: new ObjectId(bikeId) }, function(err, result) {
              if (err) {
                dbError(res, err, requestID);
                return;
              }
              ...
              if (!result) {
                ...
                res
                  .status(400)
                  .send(
                    "Invalid reservation request was made for BikeId " + bikeId
                  );
              }
            });
        ...
        if (result.matchedCount !== 1 && result.modifiedCount !== 1) {
          var msg =
            'Unexpected number of bikes changed availability! Matched: "' +
            result.matchedCount +
            '" Modified: "' +
            result.modifiedCount +
            '"';
...
function tryMongoConnect(callback, results) {
  MongoClient.connect(mongoDBConnStr, { useUnifiedTopology: true }, function(
    err,
    db
  ) {
    if (err) {
...
async.retry({ times: 10, interval: 1000 }, tryMongoConnect, function(
  err,
  result
) {
  if (err) {
